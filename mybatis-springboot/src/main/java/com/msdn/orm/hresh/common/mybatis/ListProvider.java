...
import com.msdn.orm.hresh.utils.IdUtils;
...
public class ListProvider extends MapperTemplate {
  ...
  public String updateBatchByPrimaryKeySelective(MappedStatement ms) {
    ...
    for (EntityColumn column : columnList) {
      if (!column.isId() && column.isUpdatable()) {
        ...
        sql.append(
            "         when " + idSql.toString() + " then #{i." + column.getEntityField().getName()
                + "}");
        sql.append("      </if>");
        sql.append("    </foreach>");
        sql.append("  </trim>");
  ...
  private void appendWhereIdList(StringBuilder sql, Class<?> entityClass) {
    ...
    if (columnList.size() == 1) {
      ...
      sql.append(
          "<bind name=\"_idList\" value=\"@com.msdn.orm.hresh.common.mybatis.ListProvider@notEmpty(idList)\"/>");
      sql.append("<where>");
      ...
      throw new MapperException(
          "继承 ByIdList 方法的实体类[" + entityClass.getCanonicalName() + "]中必须只有一个带有 @Id 注解的字段");
    }
  }
}
